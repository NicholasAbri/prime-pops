  ...
  Object.keys(menuPacks).forEach((packId) => {
    ...
    holder.innerHTML = `
          ...
          ${
            menuState.selectedPack === packId &&
            menuState.selectedFlavor &&
            flavorDescriptions[menuState.selectedFlavor]
              ? `
...
function updateMenuSummary() {
  ...
  document
    .querySelectorAll(".menu-pack-card")
    .forEach((card) => card.classList.remove("selected"));
  if (hasPack) {
    document
      .getElementById(`pack-${menuState.selectedPack}`)
      ?.classList.add("selected");
  }

  document.getElementById("summary-pack").textContent = hasPack
    ? pack.name
    : "Not selected";
  document.getElementById("summary-flavor").textContent =
    menuState.selectedFlavor || "Not selected";
  document.getElementById("summary-qty").textContent = hasPack
    ? menuState.quantity
    : "Not selected";
  document.getElementById("summary-total").textContent = hasPack
    ? `GH₵${total}`
    : "GH₵0";

  const largeOrderNotice = document.getElementById("large-order-notice");
  if (largeOrderNotice) {
    largeOrderNotice.classList.toggle(
      "hidden",
      !(hasPack && menuState.quantity > 20),
    );
  }
  ...
  const mobileCheckoutFlavor = document.getElementById(
    "mobile-checkout-flavor",
  );
  if (mobileCheckoutFloat && mobileCheckoutPack && mobileCheckoutFlavor) {
    const showMobilePrompt =
      readyForCheckout && menuState.checkoutPromptVisible;
    ...
    mobileCheckoutFloat.setAttribute(
      "aria-hidden",
      showMobilePrompt ? "false" : "true",
    );
    mobileCheckoutPack.textContent = readyForCheckout ? pack.name : "Pack size";
    mobileCheckoutFlavor.textContent = readyForCheckout
      ? menuState.selectedFlavor
      : "Flavor";
  }
...
function backToMenu() {
  ...
  (firstSelected || menu)?.scrollIntoView({
    behavior: "smooth",
    block: "center",
  });
  window.setTimeout(() => {
...
function revealSelectedFlavorDescription(packId) {
  window.setTimeout(() => {
    const description = document.querySelector(
      `#pack-${packId} .flavor-description-card`,
    );
  ...
  if (!menuState.selectedFlavor) {
    ...
    if (checkoutError) {
      checkoutError.textContent =
        "Please choose your preferred flavor before continuing.";
      checkoutError.classList.add("show");
    }
    updateMenuSummary();
    document
...
function closeModal() {
  const modal = document.getElementById("order-modal");
  const card = document.getElementById("modal-card");
  card.classList.add("translate-y-6");
...
function modalChangeQty(delta) {
  const next = _modalQty + delta;
  if (next < 1 || next > 99) return;
  _modalQty = next;
  document.getElementById("modal-qty").textContent = _modalQty;
  document.getElementById("modal-total").textContent =
    `GH₵${_modalPriceUnit * _modalQty}`;
}
...
function getPendingWhatsAppOrder() {
  try {
    return JSON.parse(
      safeSessionStorage.get("primepops-pending-whatsapp-order") || "null",
    );
  } catch (error) {
    return null;
  }
...
function showWhatsAppReturnModal() {
  if (safeSessionStorage.get("primepops-awaiting-whatsapp-return") !== "true")
    return;
...
function showPremiumSuccessExperience() {
  ...
  const celebration = document.getElementById("premium-celebration-stage");
  ...
  celebration.classList.remove("celebration-active", "celebration-complete");
...
function maybeShowWhatsAppReturnPrompt() {
  if (safeSessionStorage.get("primepops-awaiting-whatsapp-return") !== "true")
    return;
  const launchedAt =
    window.primePopsWhatsAppLaunchTime ||
    getPendingWhatsAppOrder()?.launchedAt ||
    0;
...
function setTheme(isDark) {
  ...
  safeLocalStorage.set("primepops-theme", isDark ? "dark" : "light");
...
document.addEventListener("DOMContentLoaded", () => {
  const saved = safeLocalStorage.get("primepops-theme");
  ...
  document.addEventListener("keydown", (event) => {
    ...
    trapDialogFocus(event, successModal, successCard, closePremiumSuccessModal);
  });
  ...
  window.addEventListener("focus", () =>
    window.setTimeout(maybeShowWhatsAppReturnPrompt, 250),
  );
